...
网页包含总览、我的收藏、同步中心和标签管理四个页面。我的收藏支持全文搜索、筛选、排序、分页和多标签交集或并集，并可选择每行 1 至 4 列的顺序网格。卡片始终按排序结果从左至右、从上至下排列；列数保存在当前浏览器，窄屏会自动减少实际列数。为避免打乱阅读顺序，不同高度卡片之间可能保留少量行内空白。图片可在查看器中缩放、旋转和复位；视频使用浏览器原生控件播放。删除推文会同步删除其媒体、原始响应和标签关联，操作不可撤销。
