...
Chrome 扩展现在只使用刷新 Likes 页面后捕获的网络响应，不再通过 DOM 读取已渲染推文，也不再提供“从当前位置继续”模式。开始同步前请先确认当前标签页是目标账号的 Likes 页面，点击“刷新并开始同步”后保持页面打开，直到同步中心显示完成。

媒体入库会校验推文媒体类型、媒体地址和服务器返回的 MIME 类型。`amplify_video_thumb` 等视频缩略图不会作为图片或视频文件保存；无法确认真实视频地址时会拒绝下载并记录失败。同步中心会列出具体推文 ID、作者昵称、@账号、原文链接、媒体序号和失败原因，可从该条记录重试或打开原文。
